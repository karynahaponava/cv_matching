...
| Ветка | Назначение | Контур |
| --- | --- | --- |
| `development` | Общая ветка интеграции изменений команды | Тестовый |
| `main` | Проверенные релизные версии | Production |
| `feature/<задача>` / `fix/<задача>` | Изолированная работа над новой функцией или исправлением | — |

Не выполняйте прямые коммиты и push в `development` или `main` — изменения
попадают в эти ветки только через Pull Request (PR).

### Новая задача или исправление

Создавайте рабочую ветку от актуальной `development`:
...
# либо: git switch -c fix/краткое-описание
```

После завершения работы выполните нужные локальные проверки, опубликуйте ветку
и создайте PR с целевой веткой `development`:

```bash
git push -u origin feature/краткое-описание
```

PR с feature/fix-веткой сливается в `development` после review и успешных
обязательных проверок. После слияния удалите рабочую ветку.

### Выпуск релиза

Когда изменения в `development` проверены и готовы к выпуску, создаётся
отдельный PR `development` → `main`. Решение о его слиянии принимает Карина.
Паша использует `main` как источник production-развёртывания.

### Уже открытые PR

Автор каждого незавершённого PR проверяет его актуальность. Активные изменения
следует перевести на целевую ветку `development`; PR в `main` допустимы только
для согласованных релизов.

Соответствие `development` тестовому контуру и `main` production-контуру —
целевая схема работы. Настройка и подтверждение CI/CD и развёртывания остаются
в зоне ответственности Паши; наличие ветки само по себе не подтверждает
